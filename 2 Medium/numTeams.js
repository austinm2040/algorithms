...
const numSplits = (s) => {
  const numSplits = (s) => {
    ...
    for (let i = 0; i < s.length; i++) {
      uniqueRight += ++right[s[i].charCodeAt() - 'a'.charCodeAt()] === 1;
    }

    for (let i = 0; i < s.length; i++) {
      uniqueLeft += ++left[s[i].charCodeAt() - 'a'.charCodeAt()] === 1;
      uniqueRight -= --right[s[i].charCodeAt() - 'a'.charCodeAt()] === 0;
